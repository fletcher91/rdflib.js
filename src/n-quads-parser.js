...
module.exports = class NQuadsParser {
  constructor(store) {
    this.store = store
  ...
  loadBuf(str) {
    ...
    let i, q, len = rawStatements.length
    ...
    for (i = 0; i < len; i++) {
      try {
         ...
         */
        leftBoundary = cleaned.indexOf(this.nnOpeningToken, leftBoundary)
        if (leftBoundary >= 0) {
          leftBoundary += this.nnOpeningTokenOffset
          graph = this.store.sym(cleaned.substring(leftBoundary, cleaned.indexOf(this.nnClosingPostfix, leftBoundary)))
        }
    ...
    // Don't potentially bust caches trying to do everything at once
    for (i = 0, len = quads.length; i < len; i++) {
      q = quads[i]
      if (q) {
